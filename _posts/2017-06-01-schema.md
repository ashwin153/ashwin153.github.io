...
# Overview
---
```Schema``` provides a runtime library for optimistically executing transactions over arbitrary transactional key-value stores and a DSL for elegantly expressing transactions using a dynamically typed schema and functionality that is absent in SQL and most other transactional libraries. As a motivating example, let's write transactions using MySQL and ```Schema``` that both solve the concurrent increment problem described above. 
