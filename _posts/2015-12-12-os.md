...
Sections of code that can be executed by multiple threads are called **critical sections**. One way to deal with race conditions is to disable preemption within critical sections. This solution is acceptable for short critical sections and is implemented as a spin lock. Before entering a short critical section, a thread spins until it successfully holds a spin lock (typically implemented using an atomic compare-and-swap instruction), disables preemption, executes the critical section, and then unlocks the spin lock. However, this solution is unacceptable for longer critical sections, because it prevents other threads from getting a chance to run.
